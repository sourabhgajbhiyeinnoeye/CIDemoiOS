...
if [ "$oclint" = "on" ] && [ "$hasObjC" = "yes" ]; then
	...
	while read word; do
		...
		# Run OCLint with the right set of compiler options
         xctool -scheme $scheme -workspace $NetVeLocity -sdk iphonesimulator ARCHS=i386   VALID_ARCHS=i386 CURRENT_ARCH=i386 ONLY_ACTIVE_ARCH=NO -reporter json-compilation-database:compile_commands.json
